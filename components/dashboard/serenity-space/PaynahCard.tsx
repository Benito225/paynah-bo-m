...
import { ITotalAccount } from "@/core/interfaces/account";
...
export default function PaynahCard({
  lang,
  className,
  onClick,
  merchant,
  ...
  useEffect(() => {
    if (accountsInfos !== undefined) {
      setBalance(accountsInfos.total_balance);
      setAvailableBalance(accountsInfos.total_skaleet_balance);
      setLoading(false);
      setAccountActionLoading(true);
    } else {
  ...
  const showLoader = () => {
    return (
      <Skeleton
        className={`h-[20px] mt-[4px] w-[13rem] bg-[#afafaf] rounded-full`}
      />
    );
  };
  ...
  return (
    <div onClick={onClick} className={`card-design ${className}`}>
      ...
      >
        <div
          className={`bg-black w-full h-full rounded-3xl absolute z-[-1] top-0 right-0`}
        ></div>
        <Image
          className={`object-cover rounded-3xl z-[-1]`}
          src={`/${lang}/images/cover-test.png`}
          fill
          alt={`cover-card`}
          priority={true}
          sizes={`100w`}
        ...
        <div
          className={`flex justify-between ${
            className ? "mb-[2.4rem]" : "mb-[3rem]"
          } `}
        >
          <div className={`inline-flex`}>
            <div>
              <h2 className={`text-[#dbdbdb] text-sm uppercase mr-2`}>
                Compte Agregés
              </h2>
              <h3 className={`text-xs text-[#afafaf] font-light`}>
                Tous les comptes
              </h3>
            </div>
            <div>
              <Form {...changeCurrency}>
                <form
                  onSubmit={changeCurrency.handleSubmit(onSubmit)}
                  className="space-y-5"
                >
                  <FormField
                      ...
                      <FormItem>
                        <FormControl>
                          <div>
                            ...
                            >
                              <SelectTrigger
                                className={`min-w-[2rem] h-[1.2rem] border-none pl-2 pr-1 rounded-full font-light text-[#afafaf] ${
          ...
          <div>
            <svg
              onClick={toggleAllBalanceView}
              className={`w-3 h-3 mt-1 cursor-pointer ${
                displayBalance ? "fill-white" : "fill-[#adadad] fill-white"
              }`}
        ...
        <div>
          <div className={`flex flex-col space-y-2`}>
            <div className={`flex flex-col`}>
              <span
                className={`${
                  className ? "text-[11px]" : "text-xs"
                } font-light text-[#afafaf]`}
              >
                Solde
              </span>
              {isLoading ? (
                ...
                <span
                  className={`font-semibold text-white ${
                    className ? "text-base" : "text-base"
                  }`}
                >
                  {displayBalance
                    ? formatCFA(balance)
                    : hiddeBalance(formatCFA(balance))}
                </span>
              )}
            </div>
            <div className={`flex flex-col`}>
              ...
              {isLoading ? (
                ...
                >
                  {displayAvailableBalance
                    ...
                    : hiddeBalance(formatCFA(availableBalance))}
                </span>
              )}
            </div>
          </div>
