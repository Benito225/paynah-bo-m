...
import { createPointOfSales } from "@/core/apis/pointOfSale";

interface AddOnlinePointOfSaleProps {
  lang: string;
  merchant: IUser;
  children: React.ReactNode;
  bankAccountsRes?: any;
  setPosLoading: (value: ((prevState: boolean) => boolean) | boolean) => void;
}

export default function AddOnlinePointOfSale({
  lang,
  merchant,
  children,
  bankAccountsRes,
  setPosLoading,
}: AddOnlinePointOfSaleProps) {
  ...
  async function onSubmitAddOnlinePointOfSale(
    values: z.infer<typeof formSchema>
  ) {
    const payload = { ...values, ...{ posType: "Online" } };
    console.log(payload);
    try {
      const response = await createPointOfSales(
        payload,
        ...
        <div>
          <div className={`rounded-t-2xl bg-white px-8 pb-4 pt-5`}>
            <div className={`flex justify-between items-center space-x-3`}>
              ...
              >{`Ajouter un point de vente en ligne`}</h2>
              <DialogClose
                onClick={() => {
                  resetModal();
                }}
        ...
        <div className={`min-h-[6rem] pt-2 pb-5 px-8`}>
          <div className={``}>
            <Form {...AddPointOfSaleForm}>
              <form
                onSubmit={AddPointOfSaleForm.handleSubmit(
                  onSubmitAddOnlinePointOfSale
                )}
                className={``}
              >
                            ...
                            <div>
                              <div className={`inline-flex space-x-3 mb-1.5`}>
                                <h3 className={`text-sm font-medium`}>
                                  Nom du point de vente en ligne
                                </h3>
                              </div>
                              <Input
                                type={`text`}
                          ...
                          <FormControl>
                            <div>
                              <div className={`inline-flex space-x-3 mb-2`}>
                                <h3 className={`text-sm font-medium`}>
                                  Compte créditeur du point de vente en ligne
                                </h3>
                              </div>
                              <Select
                                onValueChange={(value) => {
                              ...
                              >
                                <SelectTrigger
                                  className={`w-full px-4 font-light text-sm`}
                                  style={{
                                    backgroundColor: field.value
                                      ? "#fff"
                                      : "#f0f0f0",
                                ...
                                >
                                  <SelectValue placeholder="Choisissez le compte créditeur" />
                                </SelectTrigger>
                                <SelectContent
                ...
                <div className={`flex justify-center items-center`}>
                  <Button type={"submit"} className={`mt-3 w-48 text-sm`}>
                    {`Ajouter`}
                  </Button>
                </div>
              </form>
