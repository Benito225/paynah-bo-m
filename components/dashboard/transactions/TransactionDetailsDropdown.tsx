...
export const TransactionDetailsDropdown = ({ transaction, lang, children }: DropdownProps) => {
    ...
    return (
        ...
        {isOpen && (
            // <div className={`origin-top-right absolute h-24 right-0 pc:w-[88.8vw]  large:w-[78vw] shadow-lg ring-1 bg-white ml-8 ring-black ring-opacity-5 ${isOpen ? 'z-50' : 'z-10'} mt-5 border-y-0`}>
            <div className={`origin-top-right absolute h-auto right-0 w-[79.9vw] shadow-lg ring-1 bg-white ml-8 ring-black ring-opacity-5 ${isOpen ? 'z-50' : 'z-10'} mt-6 border-y-2 border-t-0 border-l-2 border-r-2 ${transaction.transaction_type.name == 'PAYIN' ? 'border-[#F7D8D8]' : 'border-[#ACEEFF]' } translate-x-9`}>
