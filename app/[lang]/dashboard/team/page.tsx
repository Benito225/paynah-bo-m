...
export default async function PointsOfSalePage({params: { lang }, searchParams}: PointsOfSaleProps) {
            ...
            <div className={`max-w-screen-2xl 2xl:max-w-[1600px] mx-auto px-4 md:px-6 lg:px-8 h-full flex flex-col`}>
                <div className={`flex justify-between items-center`}>
                    <div className={`inline-flex items-center space-x-0.5`}>
                        ...
                        <h2 className={`text-base text-black tracking-tight`}>{`Utilisateurs`}</h2>
                    </div>
                ...
                <div className={`gap-3 mt-2.5 flex-grow`}>
                    <TeamList lang={lang} searchItems={searchItems} merchant={merchant} />
                </div>
            </div>
        </>
    );
